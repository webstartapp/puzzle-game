...
export default (systems: any[]) => {
		const triggerPressEventBefore = 200;
		const triggerLongPressEventAfter = 700;
		...
		let touchMove = new Subject().pipe(
			map(e => { console.log(23, e.target); return ({ id: e.identifier, type: "move", event: e })})
		);
